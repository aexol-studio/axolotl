...
export const vaildateChatModel = (m: string) => {
  if (!(chatModels as string[]).includes(m)) {
    throw new Error(`Invalid chat model ${m} - please choose one of the following: ${chatModels.join(', ')}`);
  }
  return m as ChatModel;
};
