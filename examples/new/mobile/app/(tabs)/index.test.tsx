...
describe('app/(tabs)/index', () => {
  it('renders starter blocks and testIDs', () => {
    ...
    expect(
      screen.getByTestId('home-list-show-suspense-failure-btn'),
    ).toBeTruthy()
    expect(
      screen.getByTestId('home-list-orientation-vertical-btn'),
    ).toBeTruthy()
    expect(
      screen.getByTestId('home-list-orientation-horizontal-btn'),
    ).toBeTruthy()
    ...
    expect(screen.getByTestId('home-button-variant-outline-btn')).toBeTruthy()
    expect(screen.getByTestId('home-card-variant-compact-btn')).toBeTruthy()
    expect(screen.getByTestId('home-toggle-language-btn')).toBeTruthy()
  })
  ...
  it('switches button and card variants from controls', () => {
    render(<HomeScreen />)

    fireEvent.press(screen.getByTestId('home-button-variant-danger-btn'))
    fireEvent.press(screen.getByTestId('home-card-variant-outlined-btn'))

    expect(screen.getByTestId('home-toggle-language-btn')).toBeTruthy()
    expect(screen.getByTestId('home-showcase-card-0-btn')).toBeTruthy()
  })

  it('updates density slider via increment control', () => {
    render(<HomeScreen />)
