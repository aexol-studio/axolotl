...
const demoItems = ['Tokens', 'State views', 'Toast layer'];
...
export function useHomeScreenState() {
  ...
  const [listFailureMode, setListFailureMode] = useState<ListFailureMode>('success');
  const [listRecoveryMode, setListRecoveryMode] = useState<ListRecoveryMode>('button');
  const [listOrientation, setListOrientation] = useState<ListOrientation>('vertical');
  const [isListRefreshing, setIsListRefreshing] = useState(false);

  const nextLanguage = useMemo(() => (language === 'en' ? 'pl' : 'en'), [language]);
  ...
  const handleTriggerListFailure = (mode: Exclude<ListFailureMode, 'success'>) => {
  ...
  const homeListErrorMessage =
    listFailureMode === 'suspense' ? t('common.home.listSuspenseFailed') : t('common.home.listErrorMessage');
