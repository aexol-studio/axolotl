...
const CoreKeyboardProvider: ProviderSlot = {
  ...
  Provider: ({ children: providerChildren }) => <KeyboardProvider>{providerChildren}</KeyboardProvider>,
};

export function AppProviders({ children, integrationSlots = [] }: AppProvidersProps) {
  ...
  const CoreThemeProvider: ProviderSlot = {
    id: 'core.theme',
    Provider: ({ children: providerChildren }) => (
      <AppThemeProvider mode={themeMode} scale={uiScale} screenProfile={screenProfile} viewportRatio={viewportRatio}>
        {providerChildren}
      </AppThemeProvider>
    ),
  };
