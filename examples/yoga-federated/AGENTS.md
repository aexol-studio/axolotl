...
**IMPORTANT:** When working on frontend tasks (anything in the `frontend/` directory), you MUST spawn a sub-agent that first reads `frontend/AGENTS.md` for frontend-specific guidelines.

Frontend tasks include:

- React components
- API calls to the GraphQL backend
- State management
- Hooks and utilities
- UI/UX changes
- Type definitions for frontend data

The frontend uses **Zeus** (generated by Axolotl) for type-safe GraphQL communication with the backend. The `frontend/AGENTS.md` file contains critical information about:

- Using `Chain` for GraphQL queries and mutations
- Defining reusable `Selector` for query shapes
- Using `FromSelector` to derive TypeScript types from selectors
- Using `$` for GraphQL variables
- Best practices for type-safe frontend development

**Do not manually duplicate backend types in the frontend.** Always use `FromSelector` to derive types from Zeus selectors, ensuring frontend types stay in sync with the schema.
