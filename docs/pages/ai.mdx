...
```sh
npx OPEN_AI_API_KEY=xxx axolotl gai "schema for booking.com like system"
```
...
**prompt_info** – add baked prompt information to your
axolotl config file
...
Provide `OPEN_AI_API_KEY` env variable and run
```sh
npx OPEN_AI_API_KEY=xxx axolotl ai path/to/schema.graphql User posts "resolver to list all the posts user has created; use mongodb"
```
...
Provide `OPEN_AI_API_KEY` env variable and run
```sh
npx OPEN_AI_API_KEY=xxx axolotl fai path/to/schema.graphql User posts "component showing list of posts with title and author username"
```
...
**frontend_prompt_info** – add baked prompt information to your
axolotl config file
